...
**Phase 2 bổ sung User và Authentication** trên nền backend Phase 1; chưa xử lý
hoặc trả lời nội dung pháp luật.

## Current phase

Đã có FastAPI, cấu hình môi trường, SQLAlchemy, PostgreSQL, Alembic, Docker,
bảng users, đăng ký, đăng nhập bằng JWT và endpoint lấy user hiện tại.
Chưa có upload, LLM, RAG, embedding, vector database, frontend, social login,
...
- Giữ `api`, `core`, `db` của Phase 1; thêm `models` cho ORM, `schemas` cho hợp
  đồng request/response và `services` cho nghiệp vụ auth. Không thêm repository
...
Phase 2 có revision `0001_create_users`; `current` phải hiển thị revision này.
Migration tạo bảng `users`, unique index email và bảng kỹ thuật `alembic_version`.
...
### Trạng thái kiểm chứng trong phiên phát triển Phase 2

- Đạt: compile/cú pháp Python 3.12, cấu hình auth và che secret, schema/validation,
  UserRead không có password/hash, biên dịch model thành PostgreSQL DDL có UUID,
  timezone và unique index, `git diff --check`. Cả Compose app và Compose test
  đều đạt `docker compose ... config --quiet` (exit 0), dù Docker cảnh báo không
  đọc được file cấu hình cá nhân trong sandbox.
- Các kiểm tra schema/model dùng package có sẵn của Python 3.13; chưa xác nhận
  runtime mục tiêu Python 3.12.
- Venv hiện chưa có dependency. Đã thử import app, pytest và Alembic nhưng không
  chạy được vì thiếu package; không coi bộ test đã pass.
- Docker CLI có mặt nhưng sandbox không được truy cập Docker daemon; yêu cầu cấp
  quyền Docker và tải dependency đều bị từ chối. Chưa xác nhận Docker build,
  khởi động container, migration upgrade thực tế, JWT/hash runtime hoặc
  Swagger/API chạy thật. Các lệnh ở trên là bước kiểm chứng còn cần thực hiện.
- `.env` được Git ignore và không được track/commit; secret local không đưa vào
  `.env.example` hoặc output kiểm tra.
...
Phase 1 (backend foundation) và Phase 2 (User/Auth) đã được triển khai trong mã nguồn.
Các bước dự kiến sau: upload và metadata tài liệu; trích xuất/chunking; embedding
và retrieval; RAG/citation/hội thoại; deploy, monitoring và hardening.
Chưa triển khai Phase 3 hoặc các tính năng AI.
